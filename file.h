...
// note that many copies of the same inode may be open with different offsets. It is even possible (and that is the reason for the refcount variable) - for example, on forks or dups - that there could be two file descriptors to the same file struct. In this case, there is just one common offset, and updating any of these fds will change the common offset.
...
struct inode {
  ...
  short minor;
  short nlink; // number of links to inode in file system (the refcount equivalent for inodes)
  uint size;
